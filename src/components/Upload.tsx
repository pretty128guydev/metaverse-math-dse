...
const Upload: React.FC<UploadProps> = ({
    ...
    const handleSubmit = async (image: string) => {
        ...
        if (!image) {
            toast.error("Please capture or upload an image before submitting.", { autoClose: 3000 });
            setIsLoading(false);
            return;
        }
        ...
        try {
            ...
            toast.error("An error occurred while submitting the image.", { autoClose: 3000 });
        } finally {
    ...
    const handleTakePhotoClick = () => {
        if (cameraAccessible) {
            ...
            toast.error("Camera not accessible. Please check your device.", { autoClose: 3000 });
        }
    };

    return (
        <div className="w-[27%] flex flex-col gap-4 m-10">
